    ...
    <section id="rekam-jejak" className="section-shell" aria-labelledby="experience-title">
      <div className="mx-auto max-w-6xl">
        <div className="section-heading">
          <p className="eyebrow">03 / Rekam Jejak</p>
        ...
        <div className="timeline">
          {educationData.map((item, index) => {
            ...
            return (
              <button key={item.institution} type="button" onClick={() => setActive(selected ? null : index)} aria-expanded={selected} className="experience-card w-full text-left outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]">
                ...
                <div className={`timeline-activity ${selected ? 'is-open' : ''}`} aria-hidden={!selected}>
                  {item.activities.length > 0 && <><span><Award size={13} aria-hidden="true" />Aktivitas</span>{item.activities.map((activity) => <p key={activity}>{activity}</p>)}</>}
                </div>
              </button>
            );
          })}
