...
function clampVolume(value: unknown): number {
  return typeof value === 'number' && Number.isFinite(value) ? Math.min(1, Math.max(0, value)) : 0.65;
}

function clampIndex(value: unknown): number {
  return Number.isInteger(value) && Number(value) >= 0 && Number(value) < playlist.length ? Number(value) : 0;
}
...
export default function MusicPlayer() {
  ...
  useEffect(() => {
    try {
      ...
      // Optional persistence; audio must still work.
    }
  }, [index, volume, muted]);
    ...
    <div id="music-player-panel" className="music-player-panel" role="dialog" aria-modal="false" aria-label="Pilih musik">
      <div className="music-player-panel-head">
        <div>
          ...
          <strong>{playlist[index].title}</strong>
        </div>
        <button type="button" onClick={closePanel} className="music-player-close" aria-label="Tutup">
          <X size={15} />
        </button>
      </div>
      <div className="music-player-playlist" role="list">
        {playlist.map((song, songIndex) => (
          <button
            key={song.title}
            type="button"
            className={`music-player-song${songIndex === index ? ' is-current' : ''}`}
            onClick={() => selectTrack(songIndex)}
            role="listitem"
            aria-current={songIndex === index ? 'true' : undefined}
          >
      ...
      {state === 'error' && (
        <div className="music-player-error-inline" role="status">
          Musik gagal dimuat. Tekan play untuk mencoba lagi.
        </div>
      )}
    </div>
  ) : null;
  ...
  return (
    <div className="music-player" aria-label="Pemutar musik">
      <audio
        ...
        onPause={() => {
          const audio = audioRef.current;
          if (audio && !audio.ended) setState('paused');
        }}
        onEnded={handleEnded}
        onError={handleError}
      />
