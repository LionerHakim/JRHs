...
export default function MusicPlayer() {
  ...
  useEffect(() => {
    ...
    return () => ['play', 'pause', 'nexttrack', 'previoustrack'].forEach((action) => { try { mediaSession.setActionHandler(action as MediaSessionAction, null); } catch {} });
  }, [index, state]);
  useEffect(() => {
    if (!open) return;
  ...
  function togglePlayback() { if (state === 'playing') pause(); else if (state !== 'loading') play(); }
  function changeTrack(step: number) {
    const next = (index + step + playlist.length) % playlist.length;
    setIndex(next); const audio = audioRef.current; if (!audio) return;
    audio.pause(); audio.src = playlist[next].src; audio.currentTime = 0; audio.volume = muted ? 0 : volume; setOpen(false);
    void audio.play().then(() => setState('playing')).catch(() => setState('error'));
  }
  function selectTrack(next: number) {
    const audio = audioRef.current;
    setIndex(next); setOpen(false); if (!audio) return;
    audio.pause(); audio.src = playlist[next].src; audio.currentTime = 0; audio.volume = muted ? 0 : volume;
    void audio.play().then(() => setState('playing')).catch(() => setState('error'));
  }
  function closePanel() { setOpen(false); window.setTimeout(() => triggerRef.current?.focus(), 0); }
  ...
  return (
    <div ref={playerRef} className="music-player" aria-label="Pemutar musik">
      ...
      <div className="music-player-pill">
        ...
        <button type="button" className="music-player-track-button" onPointerDown={(event) => event.stopPropagation()} onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="music-player-panel">
      ...
      {open && (
        <div id="music-player-panel" className="music-player-panel" role="dialog" aria-label="Pilih musik">
          ...
          <div className="music-player-playlist" role="list">
            {playlist.map((song, songIndex) => <button key={song.title} type="button" className={`music-player-song${songIndex === index ? ' is-current' : ''}`} onPointerDown={(event) => event.stopPropagation()} onClick={() => selectTrack(songIndex)} role="listitem"><span className="music-song-index">{String(songIndex + 1).padStart(2, '0')}</span><span>{song.title}</span>{songIndex === index && <span className="music-song-state">●</span>}</button>)}
          </div>
          <div className="music-player-mini-volume"><label htmlFor="music-volume">Volume</label><input id="music-volume" type="range" min="0" max="1" step="0.01" value={muted ? 0 : volume} onChange={(event) => { setMuted(false); setVolume(Number(event.target.value)); }} onPointerDown={(event) => event.stopPropagation()} /></div>
        </div>
      )}
    </div>
  );
