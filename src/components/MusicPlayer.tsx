    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-1 rounded-full border border-[#E5E5E5] bg-[var(--color-paper-white)] px-1.5 py-1.5 shadow-[0_0_0_3px_rgba(247,247,247,.8)]">
        ...
        <button type="button" onClick={() => setOpen((value) => !value)} className="hidden min-h-11 max-w-36 items-center gap-2 px-1 text-left sm:flex" aria-label="Buka pemutar musik" aria-expanded={open} aria-controls="music-player-panel">
          ...
          <span className="truncate text-xs font-medium text-[var(--color-graphite)]">{state === 'error' ? 'Audio tidak tersedia' : playlist[index].title}</span>
      ...
      {open && (
        <div id="music-player-panel" ref={panelRef} className="absolute right-0 top-[calc(100%+10px)] z-[70] w-[min(21rem,calc(100vw-2rem))] rounded-2xl border border-[#E5E5E5] bg-[var(--color-paper-white)] p-3 shadow-[0_18px_50px_rgba(0,0,0,.12)]" role="dialog" aria-label="Pemutar musik VVIP+">
          <div className="flex items-start justify-between gap-3 border-b border-[#E5E5E5] px-2 pb-3">
            <div><p className="eyebrow">Pemutar musik</p><p className="mt-1 text-sm font-semibold text-[var(--color-ink-black)]">{state === 'error' ? 'Audio tidak tersedia' : playlist[index].title}</p></div>
            <button type="button" onClick={() => { setOpen(false); triggerRef.current?.focus(); }} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-[#E5E5E5] bg-[var(--color-ash-mist)] text-[var(--color-ink-black)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-signal-blue)]" aria-label="Tutup pemutar musik"><X size={16} aria-hidden="true" /></button>
          </div>
          <div className="mt-4">
            <label className="sr-only" htmlFor="music-progress">Posisi lagu</label>
            <input id="music-progress" type="range" min="0" max="100" step="0.1" value={progress} onChange={seek} disabled={!duration} className="h-1.5 w-full accent-[var(--color-signal-blue)]" />
            <div className="mt-1 flex justify-between text-[11px] text-[var(--color-smoke)]"><span>{formatTime((progress / 100) * duration)}</span><span>{formatTime(duration)}</span></div>
          </div>
          <div className="mt-3 flex items-center justify-center gap-1">
            <button type="button" onClick={() => changeTrack(-1)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full text-[var(--color-graphite)] hover:bg-[var(--color-ash-mist)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-signal-blue)]" aria-label="Lagu sebelumnya"><SkipBack size={17} aria-hidden="true" /></button>
            <button type="button" onClick={togglePlayback} className="inline-flex min-h-12 min-w-12 items-center justify-center rounded-full bg-[var(--color-ink-black)] text-[var(--color-paper-white)] active:scale-[.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-signal-blue)]" aria-label={state === 'playing' ? 'Jeda musik' : 'Putar musik'} disabled={state === 'loading'}>{state === 'playing' ? <Pause size={17} aria-hidden="true" /> : <Play size={17} aria-hidden="true" />}</button>
            <button type="button" onClick={() => changeTrack(1)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full text-[var(--color-graphite)] hover:bg-[var(--color-ash-mist)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-signal-blue)]" aria-label="Lagu berikutnya"><SkipForward size={17} aria-hidden="true" /></button>
          </div>
          <div className="mt-4 flex items-center gap-3 border-t border-[#E5E5E5] pt-4">
            {muted ? <VolumeX size={15} className="text-[var(--color-smoke)]" aria-hidden="true" /> : <Volume2 size={15} className="text-[var(--color-smoke)]" aria-hidden="true" />}
            <label className="sr-only" htmlFor="music-volume">Volume</label>
            <input id="music-volume" type="range" min="0" max="1" step="0.01" value={muted ? 0 : volume} onChange={(event) => { setMuted(false); setVolume(Number(event.target.value)); }} className="h-1.5 flex-1 accent-[var(--color-signal-blue)]" />
          </div>
          <div className="mt-4 space-y-1" role="list" aria-label="Daftar lagu">
            {playlist.map((song, songIndex) => (
              <button type="button" key={song.src} onClick={() => selectTrack(songIndex)} className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm transition active:scale-[.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--color-signal-blue)] ${songIndex === index ? 'bg-[var(--color-ash-mist)] text-[var(--color-ink-black)]' : 'text-[var(--color-smoke)] hover:bg-[var(--color-ash-mist)] hover:text-[var(--color-ink-black)]'}`} role="listitem">
                <span className="w-5 text-[11px] text-[var(--color-smoke)]">0{songIndex + 1}</span><span className="truncate">{song.title}</span>{songIndex === index && <span className="ml-auto text-[11px] font-semibold text-[var(--color-signal-blue)]">Diputar</span>}
              </button>
            ))}
          </div>
        </div>
