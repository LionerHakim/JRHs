      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden fixed inset-0 w-full h-screen bg-black/95 backdrop-blur-3xl z-[60] flex flex-col pt-24 px-6 pb-8 overflow-y-auto"
          >
            {/* Close merah */}
            <motion.button
              type="button"
            ...
            <div className="flex flex-col items-end gap-4 mt-8">
              {navLinks.map((link, index) => (
                <motion.a
                  ...
                  href={link.href}
                  onClick={() => setIsOpen(false)}
                  className="text-right text-2xl sm:text-3xl font-semibold text-white hover:text-white/80 transition-colors duration-300"
                >
                  {link.name}
                </motion.a>
              ))}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="lg:hidden fixed inset-0 w-full h-screen bg-black/95 backdrop-blur-3xl z-40 flex flex-col pt-24 px-8 pb-8 overflow-y-auto"
          >
