      ...
      <header className={`jrh-nav-shell${scrolled ? ' is-scrolled' : ''}`} aria-label="Navigasi situs">
        <nav className="jrh-nav-inner" aria-label="Navigasi utama">
          <a href={base} onClick={goHome} className="jrh-wordmark" aria-label="Kembali ke halaman utama">
            <span className="jrh-logo-text" aria-hidden="true">JRH</span>
            <span className="sr-only">JRH</span>
          </a>
          ...
          <div className="jrh-nav-right">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <a
                    key={link.href}
                    href={link.href}
                    className={`jrh-nav-link ${isActive ? 'is-active' : ''}`}
            ...
            <button
              ...
              className="jrh-menu-button"
              aria-label="Buka menu"
              aria-expanded={open}
              aria-controls="jrh-mobile-menu"
            >
      ...
      {open && (
        <div className="jrh-menu-backdrop" role="presentation">
          ...
          <aside
            ref={menuRef}
            id="jrh-mobile-menu"
            className="jrh-mobile-menu"
            role="dialog"
            aria-modal="true"
            aria-label="Navigasi seluler"
          >
            <div className="jrh-mobile-menu-head">
              <span className="jrh-mobile-menu-label">Menu</span>
              <button
                ref={closeRef}
                type="button"
                onClick={closeMenu}
            ...
            <nav className="jrh-mobile-links" aria-label="Navigasi utama seluler">
              {navLinks.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={closeMenu}
                  className={`jrh-mobile-link ${active === link.href.slice(1) ? 'is-active' : ''}`}
                >
                  <span>{link.name}</span>
