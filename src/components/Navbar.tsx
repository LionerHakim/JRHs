  ...
  return (
    <>
      <header className={`fixed inset-x-0 top-0 z-50 px-3 transition-all duration-300 sm:px-4 ${scrolled ? 'pt-3' : 'pt-4'}`} style={{ paddingTop: 'max(.75rem, env(safe-area-inset-top))' }}>
        <nav className={`mx-auto flex max-w-6xl items-center justify-between gap-3 rounded-[22px] px-2 py-1.5 transition-all duration-300 ${scrolled ? 'border border-[var(--color-line)] bg-[color-mix(in_srgb,var(--color-paper)_88%,transparent)] shadow-[var(--shadow-soft)] backdrop-blur-xl' : 'bg-transparent'}`} aria-label="Navigasi utama">
          <a href="#hero" className="group flex min-h-11 items-center gap-2 rounded-full px-2 text-[18px] font-semibold tracking-tight text-[var(--color-ink)] transition-transform hover:-translate-y-px active:scale-[.97]" aria-label="JRHs beranda"><span>JRHs</span><span className="h-1.5 w-1.5 rounded-full bg-[var(--color-accent)] transition-transform group-hover:scale-125" aria-hidden="true" /></a>
          <div className="hidden items-center gap-1 xl:flex">
            {navLinks.map((link) => { const isActive = active === link.href.slice(1); return <a key={link.href} href={link.href} className={`group relative min-h-10 rounded-full px-3.5 py-2 text-sm font-medium transition-all duration-200 active:scale-[.97] ${isActive ? 'bg-[var(--color-ash-mist)] text-[var(--color-ink)] shadow-[inset_0_1px_0_rgba(255,255,255,.25)]' : 'text-[var(--color-muted)] hover:bg-[var(--color-ash-mist)] hover:text-[var(--color-ink)]'}`} aria-current={isActive ? 'location' : undefined}>{link.name}<span className={`absolute bottom-1 left-1/2 h-0.5 -translate-x-1/2 rounded-full bg-[var(--color-accent)] transition-all duration-200 ${isActive ? 'w-3 opacity-100' : 'w-0 opacity-0 group-hover:w-2 group-hover:opacity-60'}`} /></a>; })}
          </div>
      ...
      {open && <div ref={menuRef} className="fixed inset-0 z-[60] bg-[color-mix(in_srgb,var(--color-canvas)_78%,transparent)] backdrop-blur-md xl:hidden" role="dialog" aria-modal="true" aria-label="Navigasi seluler">
        ...
        <aside className="absolute right-3 top-3 flex max-h-[calc(100svh-1.5rem)] w-[min(23rem,calc(100vw-1.5rem))] flex-col overflow-y-auto rounded-[28px] border border-[var(--color-line)] bg-[var(--color-paper)] p-3 shadow-[var(--shadow-raised)]" style={{ paddingTop: 'max(.75rem, env(safe-area-inset-top))' }}>
          <div className="flex items-center justify-between px-2 pb-3"><a href="#hero" onClick={closeMenu} className="flex min-h-11 items-center text-[18px] font-semibold tracking-tight text-[var(--color-ink)]">JRHs</a><div className="flex items-center gap-1"><ThemeToggle /><button ref={closeRef} type="button" onClick={closeMenu} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-[var(--color-line)] bg-[var(--color-control)] text-[var(--color-ink)] transition-all hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] active:scale-[.94] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]" aria-label="Tutup menu"><X size={18} aria-hidden="true" /></button></div></div>
          ...
          <p className="px-3 pb-2 pt-4 text-[10px] font-semibold uppercase tracking-[.16em] text-[var(--color-muted)]">Twilight Editorial · JRHs</p>
        </aside>
      </div>}
    </>
  );
