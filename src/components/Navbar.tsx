  ...
  return (
    <>
      <header className={`fixed inset-x-0 top-0 z-50 px-3 transition-all duration-300 sm:px-4 ${scrolled ? 'pt-3' : 'pt-4'}`} style={{ paddingTop: 'max(.75rem, env(safe-area-inset-top))' }}>
        <nav className={`mx-auto flex max-w-6xl items-center justify-between gap-2 rounded-[22px] px-2 py-1.5 transition-all duration-300 ${scrolled ? 'border border-[var(--color-line)] bg-[var(--color-paper)] shadow-[var(--shadow-soft)]' : 'bg-transparent'}`} aria-label="Navigasi utama">
          ...
          <div className="ml-auto flex items-center gap-1">
            <MusicPlayer />
            <button ref={openerRef} type="button" onClick={() => setOpen(true)} className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-[var(--color-line)] bg-[var(--color-control)] text-[var(--color-ink)] shadow-[var(--shadow-soft)] transition-all hover:-translate-y-px hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] active:scale-[.94] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]" aria-label="Buka menu" aria-expanded={open}><Menu size={18} aria-hidden="true" /></button>
          </div>
        </nav>
      </header>

      {open && <div ref={menuRef} className="fixed inset-0 z-[60] bg-[var(--color-canvas)]/94" role="dialog" aria-modal="true" aria-label="Navigasi seluler">
        ...
        <aside className="absolute right-3 top-3 flex max-h-[calc(100svh-1.5rem)] w-[min(23rem,calc(100vw-1.5rem))] flex-col overflow-y-auto rounded-[28px] border border-[var(--color-line)] bg-[var(--color-paper)] p-3 shadow-[var(--shadow-raised)]" style={{ paddingTop: 'max(.75rem, env(safe-area-inset-top))' }}>
          ...
          <nav className="rounded-[22px] border border-[var(--color-line)] bg-[var(--color-canvas)] p-1.5" aria-label="Navigasi utama seluler">{navLinks.map((link, index) => { const isActive = active === link.href.slice(1); return <a key={link.href} href={link.href} onClick={closeMenu} className={`group flex min-h-14 items-center justify-between rounded-[17px] px-4 text-[15px] font-semibold transition-all duration-200 active:scale-[.985] ${isActive ? 'bg-[var(--color-paper)] text-[var(--color-ink)] shadow-[var(--shadow-soft)]' : 'text-[var(--color-muted)] hover:bg-[var(--color-paper)] hover:text-[var(--color-ink)]'}`} aria-current={isActive ? 'page' : undefined}><span className="flex items-center gap-3"><span className="font-mono text-[10px] tracking-widest text-[var(--color-accent)]">0{index + 1}</span>{link.name}</span><span className={`h-1.5 w-1.5 rounded-full bg-[var(--color-accent)] transition-transform ${isActive ? 'scale-100' : 'scale-0 group-hover:scale-75'}`} /></a>; })}</nav>
        </aside>
      </div>}
    </>
  );
