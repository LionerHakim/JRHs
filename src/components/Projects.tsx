    ...
    <section id="projects" className="section-shell projects-section" aria-labelledby="projects-title">
      ...
      <div className="projects-v5-grid" aria-label="Daftar project">
        {projects.map((project, index) => <article key={project.title} className={`project-v5-card project-v5-card-${index + 1}`}>
          ...
          <div className="project-v5-content"><div><span className="project-v5-category">{project.category}</span><h3>{project.title}</h3><p>{project.description}</p><span className="project-v5-role">{project.role}</span></div><div className="project-v5-actions"><button type="button" className="project-v5-detail" onClick={() => setSelected(project)} aria-label={`Lihat detail ${project.title}`}>Info Detail</button><a className="project-v5-link" href={project.url} target="_blank" rel="noopener noreferrer" aria-label={`Buka source ${project.title}`}>Source <ArrowUpRight size={15} aria-hidden="true" /></a></div></div>
        </article>)}
      </div>
      <p className="projects-note" aria-hidden="true">Geser di mobile untuk melihat project berikutnya.</p>
    </section>
    {selected && <div className="project-detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelected(null); }}><article className="project-detail" role="dialog" aria-modal="true" aria-labelledby="project-detail-title"><button type="button" className="project-detail-close" onClick={() => setSelected(null)} aria-label="Tutup detail project"><X size={18} aria-hidden="true" /></button><span className="eyebrow">{selected.category}</span><h2 id="project-detail-title">{selected.title}</h2><p className="project-detail-copy">{selected.detail}</p><dl><div><dt>Role</dt><dd>{selected.role}</dd></div><div><dt>Stack</dt><dd>{selected.stack}</dd></div></dl><a className="project-v5-link" href={selected.url} target="_blank" rel="noopener noreferrer">Buka Source <ArrowUpRight size={15} aria-hidden="true" /></a></article></div>}
  </>;
}
