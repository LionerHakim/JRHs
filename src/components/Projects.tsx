...
export default function Projects() {
  ...
  const openProject = (project: (typeof projects)[number], opener: HTMLButtonElement) => {
    ...
    <section id="projects" className="section-shell" aria-labelledby="projects-title">
          ...
          {projects.map((project) => (
            <article key={project.title} className="project-card">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                ...
                <button ref={openerRef} type="button" onClick={(event) => openProject(project, event.currentTarget)} className="portal-link shrink-0 border border-[var(--color-line)] bg-[var(--color-paper)] px-4 text-sm font-semibold text-[var(--color-ink)] transition hover:-translate-y-0.5 hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent)] active:scale-[.97]">Info Detail</button>
              </div>
            </article>
          ))}
        </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-[100]">
          <button type="button" className="absolute inset-0 h-full w-full bg-black/60 backdrop-blur-[1px]" onClick={closeProject} aria-label="Tutup detail proyek" />
